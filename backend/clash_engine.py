...
def check_clash_for_permit(new_permit: Permit, db: Session):
    ...
    other_permits = db.query(Permit).filter(
        ...
        Permit.status.in_(["SUBMITTED", "PENDING_REVIEW", "APPROVED", "IN_PROGRESS", "CLASH_DETECTED", "EMERGENCY"])
    ).all()
    ...
    for other in other_permits:
        ...
        if new_buffered.intersects(other_buffered):
            ...
            # Check Depth Overlap
            depth_overlap = check_depth_conflict(new_permit.depth_meters, other.depth_meters)
        ...
        for road in resurfaced_roads:
            ...
            if new_buffered.intersects(road_buffered):
                ...
                if isinstance(permit_start, str):
                    permit_start = datetime.datetime.strptime(permit_start, "%Y-%m-%d").date()
                    
                months_since_resurface = (permit_start - road.last_resurfaced_date).days / 30.0
                ...
                if months_since_resurface < 24.0:
                    ...
                    # We store it with conflicting_permit_id = -road.id (negative ID represents road conflict)
                    clash_obj = Clash(
                        permit_id=new_permit.id,
                        conflicting_permit_id=-road.id,
